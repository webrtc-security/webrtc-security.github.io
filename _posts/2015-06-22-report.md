...
P2P encryption is easy to envisage and setup, but we must also consider the alternative communication structure of TURN briefly. 

Even under TURN communication, the sent data is encrypted at the end points. A TURN server's purpose is simply the relay of WebRTC data between parties in a call, and will only parse the UDP layer of a WebRTC packet for routing purposes. Servers will not decode the application data layer in order to route packets, and therefore they do not touch the DTLS encryption. 
...
Applications with a focus upon security/confidentiality would ensure that their signalling layer is over a secure protocol such as SIPS / HTTPS / WSS.
