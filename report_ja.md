...
WebRTC登場以前は、音声や映像の取得にはサードパーティのプラグイン（例えばFlashやSilverlight）が必要だった。だが、HTML5の時代になり、多くのデバイスについて直接アクセスする手段が出現し、JavaScriptから、それらのデバイスを利用できるようにあった。
...
発信者が、ある着信先に通信を開始する場合、ブラウザはRTCPeerConnectionのインスタンスをまず作成する。このとき、自身が作成するSDP(Session Description Protocol)を、通信相手と交換する。
...
しかし、TURNサーバを介することで、メディアの品質・レイテンシが低下する可能性もありうる。
...
全てのWebRTCの技術は、WebRTC互換のブラウザ(例えば、ChromeやFirefox)をインストールことで導入される。
...
また、TLSは信頼性のあるデータ転送を提供するTCPの上に動作するよう設計されているが、
...
本レポートでは、TURNを利用した場合の、暗号化通信についても考えておく必要がある。
...
これらの問題が浮上した結果、強化されたセキュリティ対策の実装方法が必要となっている。
...
もっともセキュアな技術として扱われる。
暗号化が標準で利用される性質により、通信は常にプライベートな状態にある。
